...
func format(str string) string {
	for i := 0; i < len(str)-1; i++ {
		if i >= 0 && i < len(str)-1 {
			if IsOpeningBracket(rune(str[i])) {
				if i > 0 && str[i-1] != ' ' && !IsOpeningBracket(rune(str[i-1])) {
			...
			if IsClosingBracket(rune(str[i] ))  {
				if i > 0 && str[i-1] == ' ' {
					str = RemoveStringElem(str, i-1)
					i--
				}
...
func IsBracket(r rune)bool {
	return IsOpeningBracket(r)||IsClosingBracket(r)
}
				...
				if isQuote(rune(s[i])) {
					...
					case squote:
						if qa {
							...
							if s[i+1] != ' ' && rune(s[i+1]) != squote {
								s = addSpaceBetweenString(s, i)
